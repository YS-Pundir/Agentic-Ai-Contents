# Lecture Script: Lists, Iteration, and Functions
...
**How to use this file:** Each block lists *what happens in the room* and its **duration in minutes**. This is timing and facilitation only—definitions, code, and diagrams stay with your **Lecture Notes** (slides, notebook, or board). Skim headings aloud; let visuals and live code carry the structure. **List comprehensions are out of scope for this session** (not in the current notes); use ordinary loops for transforms and filters.

**Break (only pause in this plan):** After **roughly 70–80 minutes** of session clock time (after **iterating through lists**), take **one** pause of **5–8 minutes**, then continue. Every numbered section below is **teaching or activity**—there are **no** “break” rows in the numbered list.

---

## 1. Welcome and roadmap (5 minutes)

- Greet; confirm VS Code (or your standard IDE) is opened and ready to learn.
- State headline outcomes: **create and shape lists**, **index and slice** safely, **iterate** with `for` / `enumerate` / `zip`, then **`def` functions** with **parameters**, **`return`**, and **defaults** (including mutable-default caution).
- Note explicitly that **list comprehensions are deferred**—learners will build lists with loops and the patterns in the notes; comprehensions can come in a later module if your program schedules them.
...
## 2. Creating and initializing lists (14 minutes)
...
## 3. Accessing elements with indexing (14 minutes)
...
## 4. Slicing: sublists and slice assignment (21 minutes)
...
## 5. Iterating through lists (23 minutes)

- Frame as **lists + loops**; show **iteration flowchart** from notes.
- **Live:** `for item in list`, **conditional inside loop**, **accumulator** (sum); optionally show building a **new list** with `.append()` where the notes illustrate filtering or mapping—this replaces “comprehension in one line” pedagogy with explicit loops.
...
## 6. Defining functions: `def`, naming, calls, composition (16 minutes)
...
## 7. Function parameters (12 minutes)
...
## 9. Default parameters and the mutable-default pitfall (9 minutes)
...
**Sum of numbered teaching segments:** 5 + 14 + 14 + 21 + 23 + 16 + 12 + 13 + 9 = **127 minutes** of teaching, plus **5–8 minutes** break ≈ **132–135 minutes** total session clock. Use spare time in **5** or **9** for Q&A.

---

### Timing flex

- **If behind:** Trim **4** (one slice-assignment demo only) or **5** (`for-else` / extra 2D pattern as reading); keep **8–9** (`return` vs `print` + mutable defaults) intact.  
- **If ahead:** Add minutes to **5** (second accumulated / filtered list example with a plain loop), **8** (compose two functions), or **9** (learners predict mutable-default output before reveal).  
- **If session block is shorter than 2h15:** Drop optional sub-bullets in **2** and **4** first.
