...
| | Search engine | LLM |
|---|---|---|
| **Core job** | Find existing pages | **Generate** new text |
| **Same query again** | Same top results (mostly) | Answer **may change** |
| **Can invent content?** | No — it returns what exists | **Yes** — fluency ≠ truth |
| **Creativity** | Low | High |

- **Temperature** (a setting from 0 to 2) controls randomness: low = safer, repeatable choices; high = more surprising, sometimes nonsense. You will tune this when building apps.
- **Common mistake:** Treating a confident tone as proof of correctness — the model is optimised to sound right, not to be right.

### Quick Activity — The Sentence Completion Game

Complete each starter with the **first word that comes to mind**. Notice which feel "forced" to one answer vs open to many.

1. *"The capital of India is ___"*
2. *"She opened the door and saw a ___"*
3. *"To make chai, first boil ___"*
4. *"The Eiffel Tower is located in ___"*
5. *"Once upon a ___"*

**Your notes:** Starters 1, 4, and 5 have one dominant answer — high **probability** for a specific token. Starter 2 has many valid continuations — the model **samples** from several plausible paths. That is why creative prompts vary run to run, while factual prompts are more stable (but still not guaranteed correct).

---

## When LLMs Fail — Hallucinations and Other Failure Modes

LLMs are powerful and **unsafe to trust blindly**. Know the main failure modes so you design around them.
...
- **Real-Life Example:** A student who never says *"I don't know"* — they invent a citation that looks real until you check the library shelf.
...
### Rule-Based vs LLM — A Tiny Python Demo

This script shows a **keyword router** (rule-based) next to why **generation** is a different beast — rules return fixed strings; LLMs need separate guardrails.

```python
# No ML library needed — pure Python rule-based demo

def rule_based_support(message):  # Define a simple keyword router
    message_lower = message.lower()  # Normalise case for matching
    if "refund" in message_lower:  # Keyword rule for billing
        return "Routing to billing team."  # Fixed response path
    if "track" in message_lower:  # Keyword rule for shipping
        return "Routing to shipping team."  # Fixed response path
    return "Sorry, I did not understand."  # Fallback when no keyword hits

# Test messages — same intent, different words
messages = [  # List of customer messages to try
    "I need a refund for order 99.",  # Hits keyword — works
    "Please return my money.",  # Same intent — rule fails
    "Where is my package?",  # No rule — fallback
]

for msg in messages:  # Loop through each test message
    print(f"Customer: {msg}")  # Show the input
    print(f"Bot: {rule_based_support(msg)}\n")  # Show rule-based output

# Key lesson printed as a comment in your notebook:
# An LLM might paraphrase intent correctly — but could also hallucinate a policy.
# Production systems combine LLM language skill with rules, retrieval, and human review.
```

**How the code works:**

- `message.lower()` makes keyword matching case-insensitive.
- Two `if` checks implement a **brittle** rules engine — fast and predictable, but blind to paraphrase.
- The third message has no matching keyword → generic fallback.
- Compare mentally with an LLM: better paraphrase handling, but needs **hallucination guardrails** the rule engine does not need for routing-only tasks.
...
In upcoming sessions you will learn how to **steer** these models with better prompts and how to **ground** them in real documents so answers cite evidence instead of imagination.
...
| **Temperature** | Randomness dial (0–2) on token sampling — low = steady, high = creative |
