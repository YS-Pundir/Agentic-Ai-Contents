...
This session builds on that foundation: how **embeddings** work as semantic coordinates, how **similarity scores** help you judge matches, and how **Chroma** stores and searches vectors (**FAISS** is a common alternative). You will **embed sample text**, **store** it, and run a **top-k semantic search** — the retrieve step a RAG pipeline reuses.
...
- **Real-Life Example:** SQL finds a ticket by **PNR** (exact key). Vector search finds complaints **like** this one (nearest meaning).
...
| **FAISS** | Common alternative; same embed → store → query idea |
| **pgvector** | Vectors inside PostgreSQL — mention only |
...
| **Chroma / FAISS** | Tool | Vector stores (Chroma in lab) |
