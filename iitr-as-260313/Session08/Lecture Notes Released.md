...
- **Official Definition (high level):** A **Transformer** is a type of neural network architecture designed to process text so that the model can use **the full prompt at once** and relate words across the whole input — instead of walking through the sentence one word at a time and losing early context.
...
- **Before Transformers (older models like RNN/LSTM):** These models read text *one word at a time, in sequence*, like reading a sentence word by word in strict order. By the time they reached the end of a long sentence, they had "forgotten" the beginning — so the meaning of the whole instruction could break (for example, forgetting that you asked for a PDF, not just an explanation).
- **With Transformers:** The model takes in the *entire input together*. Each part of the text can connect to the rest, so long instructions and long context stay coherent. A deeper look at *how* that works inside the model (including the famous **attention** idea from the paper) comes in a later session.
...
- LLMs are built on the **Transformer architecture**, which processes the **whole input at once** so context stays coherent across long prompts (more detail in a later session).
...
- **Text in, text out** — Treat everything the LLM does as that single pattern; capabilities (Q&A, code, summaries) are all variations of it.
- **The LLM is the agent’s brain** — Planning, reasoning, and deciding what to do next happen through prompts sent to the model; tools carry out actions.
- **Prompts steer behavior** — Same model, different instructions → different outputs; clear prompts matter for quality and cost.
- **Strengths vs. limits** — Strong at language, breadth, and flexible formats; weak when facts must be exact, current, or mathematically precise without tools.
- **Hallucinations are real** — Confident wrong answers are possible; design agents to verify (search, APIs, code) when stakes are high.
- **Providers and APIs** — You use models built by companies like OpenAI, Google DeepMind, Anthropic, or Meta by calling their **APIs** from your code — you do not run the full model on your laptop.
- **Tokens drive limits and cost** — Text is chunked into tokens; long prompts and long outputs use more tokens, hit context limits sooner, and cost more.
- **Transformers (intuition)** — Modern LLMs rely on architectures that take in the **whole prompt together**, unlike older step-by-step models that lost early context; inner details come later in the course.
- **Failure-aware design** — Combine LLM reasoning with the right tools and checks so wrong or inconsistent outputs do not become wrong real-world actions.
...
| **Transformer** | The neural network architecture that powers all modern LLMs | Introduced in Google's 2017 "Attention Is All You Need" paper; takes in the full prompt so long context stays usable |
