...
Across the last few classes you moved from **vector search** (finding similar text in Chroma) to that **minimal RAG loop**, and now to a **full ingestion pipeline** with real policy files. Today you replace hardcoded strings with **TXT** and **PDF** documents, add **document loaders** and **chunking**, then plug the result into the **same retrieve → generate loop**.
...
- **Chunk defaults:** **100 words** per chunk, **20 words** overlap — keeps split rules from losing context (e.g. *"not eligible unless defective"*). The live walkthrough used a basic **100-word fixed** strategy; overlap preserves context across chunk boundaries when you enable it in code.
- **Other strategies** such as **recursive** or **paragraph-based** chunking can replace fixed word counts in a **later** session — today you use a sensible default and **inspect Rank 1 text** in the terminal.
- **Too large / too small chunks** hurt retrieval; systematic tuning comes in a **later** session.
...
Install the packages needed to run the full pipeline:

```bash
pip install chromadb sentence-transformers groq python-dotenv pypdf
```

Use **`pip3 install`** if your machine maps `python3` to the environment you run the script in.

### Option A — Clone from GitHub (recommended in class)

1. Clone the course repository using the GitHub link shared in class (`git clone <repo-url>`).
2. Open the project folder in **VS Code** (or your editor).
3. Confirm the policy folder is named exactly **`policy_documents`** — a different name breaks the loader path.
4. The instructor copy may name the script **`code.py`**; these notes use **`rag_pipeline.py`** — the logic is the same.
5. Add your **`GROQ_API_KEY`** to **`.env`** in the project root (recap from the minimal RAG lab).

### Option B — Create the project locally
...
### Policy Files (create these four if not cloning)
...
- **`top_k=3`** because each policy file now produces **multiple chunks** — Rank 1 is the closest match, but **all three ranks** are sent to Groq so the model can combine evidence when one chunk alone is incomplete.
- **`distance`** scores in the terminal show how close each chunk is to the query — a lower value usually means a stronger match (e.g. **0.48** for Rank 1 vs **0.65** for Rank 2).
...
Use **`python3 rag_pipeline.py`** if that is how Python is installed on your machine. If you cloned the instructor repo, run the script name present there (often **`code.py`**).
...
- **Ingestion is new today; the RAG loop is not** — loaders and chunking feed the same BGE + Chroma + Groq pipeline you built in the minimal lab.
- **Chunk size and overlap** control which rules retrieval surfaces; inspect retrieved text and **distance** scores every run.
- **Metadata** (`category`, `source`) helps you trace which policy file supplied each excerpt.
- **`top_k=3`** sends multiple ranked excerpts to the generator — do not rely on Rank 1 alone when the answer spans more than one rule.
...
| **`top_k=3`** | Retrieve three excerpts per question; all ranks feed the grounded prompt |
| **`distance`** | Similarity score per retrieved chunk — lower usually means closer match |
| **`answer_with_rag`** | Retrieve → print ranks → generate grounded answer |
| **`git clone`** | Copy the instructor project (including `policy_documents`) from GitHub |
