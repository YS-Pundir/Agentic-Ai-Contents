...
| **Generator** | Turn retrieved text + question into a polite reply | **OpenAI chat API** (or equivalent LLM API) |
...
Create a dedicated folder for today's lab. You need the same two libraries from your vector search lab, plus the OpenAI client for generation.
...
pip install -U sentence-transformers  # Local embedding model for retriever
pip install openai  # OpenAI Python client for the generator step
```

**How the code works:**

- **`chromadb`** provides **`PersistentClient`**, **`get_or_create_collection`**, **`upsert`**, and **`query`** — your retriever storage layer.
- **`sentence-transformers`** loads **`all-MiniLM-L6-v2`** so you embed policy text and customer questions on your laptop without a separate embedding API.
- **`openai`** sends the grounded prompt to an LLM and returns the final answer text.

![Lab stack for Session 20 — chromadb for persistent vectors, sentence-transformers for local embeddings, and OpenAI for the generator; API key stays in the environment, not in Git](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-as-260313/module2/session20/session20-04-lab-stack.png)

### API Key for the Generator

The **retriever** runs locally. The **generator** calls a hosted LLM through an API.

- Set an environment variable before running the script: `export OPENAI_API_KEY="your-key-here"` on Mac/Linux.
...
- **In Simple Words:** The **password** your script uses to talk to OpenAI's servers.
...
- [ ] `OPENAI_API_KEY` is set in the terminal session you will use to run the script
...
import os  # Read environment variables like OPENAI_API_KEY
import chromadb  # Vector database for storing and searching policy chunks
from sentence_transformers import SentenceTransformer  # Local embedding model for retriever
from openai import OpenAI  # Client for LLM generation API calls
...
EMBEDDING_MODEL_NAME = "all-MiniLM-L6-v2"  # Same model as the previous Chroma lab

# LLM model name for generation — course-friendly default; your org may allow a different name
GENERATION_MODEL_NAME = "gpt-4o"  # OpenAI chat model used as the generator
```
...
- **`EMBEDDING_MODEL_NAME`** must match what you used when indexing — the **same model rule** from vector search still applies in RAG.
...
def generate_grounded_answer(client: OpenAI, user_query: str, retrieved_chunks: List[Dict[str, Any]]) -> str:
...
- System + user **messages** follow the chat API pattern you saw when learning how LLMs are accessed remotely.
...
```python
def generate_llm_only_answer(client: OpenAI, user_query: str) -> str:
    # Same LLM — but NO policy excerpts in the prompt (ungrounded baseline)
    response = client.chat.completions.create(
        model=GENERATION_MODEL_NAME,  # Same generator model for fair comparison
        messages=[
            {
...
def main():
    ...
    client = OpenAI()  # Generator API handle

    # Load embedding model and connect to Chroma collection
    model = create_embedding_model()  # SentenceTransformer for retriever
...
export OPENAI_API_KEY="your-key-here"  # Set API key in this terminal session
...
| **`all-MiniLM-L6-v2`** | Local embedding model — same for index and query |
...
| **`openai.OpenAI()`** | Client for hosted LLM generation API calls |
| **`chat.completions.create`** | Chat API pattern for sending system + user messages to the generator |
| **`gpt-4o`** | Example generation model name used in the lab script |
| **`OPENAI_API_KEY`** | Environment variable holding your secret API token |
