...
![Symmetric encryption uses one shared secret key for both directions; asymmetric encryption uses a public key to encrypt and a private key to decrypt — solving the key-exchange problem](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-01-symmetric-vs-asymmetric.png)
...
![Hashing is one-way and fixed-length — same input always gives the same hash, and a tiny change causes a completely different output (avalanche effect)](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-02-hashing-properties.png)

In class, a live SHA-256 demo on an online hash calculator showed this behaviour: adding or removing a single space in `hello world` produced a completely different hash — the same idea used in SSL certificate checks, forensics device verification, and tamper detection.
...
![Hashing is irreversible with fixed-length output for integrity; encryption is reversible with a key and grows with input — use each for its own purpose](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-03-hashing-vs-encryption.png)
...
**The 2012 LinkedIn breach** brought this risk into the real world: 6.5 million user passwords were leaked, hashed with **MD5** and without salting. Attackers cracked them rapidly using pre-computed rainbow tables. This breach is a landmark example of what weak hashing costs in practice.

![Unsalted hashes face hash collisions, rainbow-table lookups, and GPU brute force at billions of hashes per second — as seen in the 2012 LinkedIn breach](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-04-attack-vectors.png)
...
![Password salting: generate a unique random salt, combine it with the password, hash the result, and store salt plus hash together — defeating rainbow tables](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-05-password-salting.png)
...
![Purpose-built password hashes ranked by OWASP: Argon2id first, then scrypt, bcrypt, and PBKDF2 for FIPS — never use fast SHA-256 or MD5 for passwords](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-06-password-algorithms.png)
...
**Practice (as in class):** Use an online password-strength checker and a password-hasher comparison site to try weak passwords (`admin1234`, `quality`, `123456789`) versus stronger ones with bcrypt, scrypt, and PBKDF2 (~600,000 iterations for FIPS-style PBKDF2). Notice how entropy, crack-time estimates, and output formats change.
...
**AES-256** is a widely used symmetric encryption algorithm. In class, encrypting `hello world` with a 256-bit key on an online AES tool produced ciphertext noticeably longer than the plaintext — and the output grew further as the input grew. This contrasts directly with hashing, whose output is always fixed-length regardless of input size.
...
![Secret management tiers: .env plus .gitignore in development, Vault or cloud secrets managers in production, and scheduled key rotation every 30–90 days](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-07-secret-management.png)
...
![Secure error handling: return generic messages to users while logging full details internally — never expose stack traces or secrets in API responses](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-08-secure-error-handling.png)
...
The lab (`lab/app.py`) implements both insecure patterns (used to trigger static analysis tools) and secure patterns side by side, using three Python libraries: **`hashlib`** (built-in cryptographic hashing), **`bcrypt`** (password-hashing library), and **`secrets`** (cryptographically secure random token generation). The instructor shared a **`lab-security-scan`** GitHub repository with this code — clone it to run the lab locally or on your own GitHub account.
...
![Automated security scanning in CI/CD: Bandit for Python and Semgrep for multi-language SAST catch weak hashes and hard-coded secrets on every commit](https://s13n-curr-images-bucket.s3.ap-south-1.amazonaws.com/iitr-cyb-2510/may26-crypto/cyb-may26-09-static-analysis.png)

**CI/CD integration (example in repo).** The shared repository includes a GitHub Actions workflow (`.github/workflows/security-scan.yml`) that runs on Ubuntu, installs Bandit and Semgrep, scans the codebase, and uploads findings as JSON artifacts. Uploading the project to your own GitHub account triggers the same scan — the workflow was walked through in class as a reference pattern, not run live on GitHub during the session.
...
## Quick Reference — Commands, Tools, and Terms

| Item | What it is / How to use it |
|---|---|
| **Symmetric encryption** | One shared secret key for encrypt and decrypt (e.g. AES-256). |
| **Asymmetric encryption** | Public key encrypts; private key decrypts; based on hard prime factorization. |
| **Hashing** | One-way, fixed-length output; for integrity and password verification — not confidentiality. |
| **Salting** | Unique random value per password before hash; defeats rainbow tables. |
| **Argon2id / bcrypt / scrypt / PBKDF2** | Purpose-built password hashes; prefer Argon2id; PBKDF2 when FIPS is required (~600K iterations). |
| **CrackStation** | Online lookup demo for unsalted SHA-256 hashes (e.g. `admin1234`). |
| **Bandit** | `bandit -r lab/ --severity-level medium` — Python SAST. |
| **Semgrep** | `semgrep --config auto lab/` — multi-language SAST (187 rules in class demo). |
| **OWASP salt minimum** | 16 bytes (16 chars in most languages; 8 chars in Java). |
| **Key rotation** | Production / internet-exposed: ~30–60 days; internal: ~90 days (org policy may vary). |
| **Secure login response** | Generic message such as `Invalid credentials` — never stack traces or stored hashes. |