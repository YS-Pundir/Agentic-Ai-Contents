# Assignment — Objective (Session 34: Ollama — Exploring Another World of LLMs)
...
**Explanation:** **`ollama -v`** prints the installed version — the standard verification step from setup docs. **A** and **D** are not the documented verify commands. **C** installs a Python package, not the Ollama runtime itself.
...
**Explanation:** **A–C** match the taught local script pattern. **D** is false — local Ollama uses plain **`requests`**, not the OpenAI SDK.
...
- D) After `ollama stop <model>`, the runtime can **never** start that model again, even if an API request arrives  

**Correct options:** **A, B, C**

**Explanation:** **A**, **B**, and **C** are standard CLI and security behaviour. **D** is false — Ollama may **auto-start** a model to serve an API request even after a stop, as noted in class.
